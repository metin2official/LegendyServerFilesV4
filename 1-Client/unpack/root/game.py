...
class GameWindow(ui.ScriptWindow):
	...
	def __BuildKeyDict(self):
		...
		onClickKeyDict[app.DIK_Y] = lambda: self.__PressOfflineShop()

		self.onClickKeyDict=onClickKeyDict

	if app.__ENABLE_NEW_OFFLINESHOP__:
		def __PressOfflineShop(self):
			if self.Offlineshop:
	...
	def BloktanCikartGuiGG(self,gDurum):
		if self.interface:
			self.interface.BloktanKurtarGuiGG(True)
	...
	def OnUpdate(self):
		...
		# if app.__ENABLE_NEW_OFFLINESHOP__:
			# if constInfo.offlineshop == 1:
				# self.__PressOfflineShop()
				# constInfo.offlineshop = 0
	...
	if app.ENABLE_KILL_STATISTICS:
		def ReceiveKillStatisticsPacket(self, j, sh, ch, t, td, dw, dl, b, st, mb, top_damage):
			constInfo.KILL_STATISTICS_DATA = [int(j), int(sh), int(ch), int(t), int(td), int(dw), int(dl), int(b), int(st), int(mb), int(top_damage),]
