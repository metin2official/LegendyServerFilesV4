...
ENABLE_WEAPON_EVOLUTION_SYSTEM = 0

try:
	if app.ENABLE_WEAPON_EVOLUTION_SYSTEM:
		ENABLE_WEAPON_EVOLUTION_SYSTEM = 1
except:
	pass
try:
...
def MakeSlotInfo(window, slotIndex, yang, cheque=0):
	...
	if ENABLE_WEAPON_EVOLUTION_SYSTEM:
		res['evolution'] = player.GetItemEvolution(window, slotIndex)
...
class Slot(ui.Window):
	...
	def __loadBackground(self, isSold):
		...
		bg.LoadImage("offlineshop/slot/new_base.tga")
		bg.SetParent(self)
		bg.SetPosition(0,0)
		bg.Show()
...
#
# class ItemTableWithScrollbar(ui.Window):
#
# 	def __init__(self, columns, rows, width, height):
# 		self.rows  		= rows
# 		self.columns	= columns
# 		self.slots 		= [[Slot() for x in xrange(rows)] for y in xrange(columns)]
#
# 		self.width		= width
...
# 		self.scrollbar.Show()
#
# 		offsetx = int(self.width/self.columns)
# 		offsety = int(self.height/self.rows)
#
# 		for col in xrange(self.columns):
# 			for row in xrange(self.rows):
# 				x,y = (offsetx*col , offsety* row)
# 				self.slots[col][row].SetPosition(x,y)
# 				self.slots.Show()
#
#
# 	def __del__(self):
...
class NewOfflineShopBoard(ui.ScriptWindow):
	...
	def __MakeCreateShopItemsTable(self):
		...
		table = TableWindowWithScrollbar(15*40, 3*99, 15,3, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
	...
	def __MakeMyShopItemsTable(self):
		...
		table = TableWindowWithScrollbar(15*40.2, 3*130, 15,3, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
	...
	def __MakeOpenAuctionOffersTable(self):
		...
		table = TableWindowWithScrollbar(14*40, 3*99, 1, 20, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
	...
	def __MakeAuctionListTable(self):
		...
		table = TableWindowWithScrollbar(14*40, 4*99, 1, 20, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
	...
	def __MakeSearchFilterCheckBoxes(self):
		...
		for k, v in positions.items():
			checkbox = ui.OffshopCheckBox({'base' : "offlineshop/checkbox/base.png", 'tip' : "offlineshop/checkbox/tip.png",})
	...
	def __MakeSearchFilterResultItemsTable(self):
		...
		table = TableWindowWithScrollbar(15*40.2, 3*99, 15, 3, TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
	...
	def __MakeSearchHistoryTable(self):
		...
		table = TableWindowWithScrollbar(15*40.2, 4*99, 1, 20 , TableWindowWithScrollbar.SCROLLBAR_VERTICAL)
		table.SetParent(board)
	...
	def __MakeCreateAuctionSlot(self):
		if self.CreateAuctionSlot:
			self.CreateAuctionSlot.Destroy() # patch create auction slot check
	...
	def __OnClickMyAuctionPage(self):
		...
		#self.EnableRefreshSymbol()
		#self.__OnCheckCreateAuctionSlot() # patch create auction slot check
	...
	def OpenShopItem_SetValue( self, key,	index,	*args):
		...
		elif key == "vnum" or key == 'trans' or key == 'evolution':
			self.ShopItemForSale[index][key] = args[0]
	...
	def ShopFilterResultItem_SetValue( self,  key, index, *args):
		if key in ( "id", "vnum", "count", "price", "owner", 'trans', 'cheque', 'evolution'):
	...
	def __IsSaleableSlot(self, win , pos):
		...
		if not win in (player.INVENTORY, player.DRAGON_SOUL_INVENTORY , player.UPGRADE_INVENTORY, player.BOOK_INVENTORY, player.STONE_INVENTORY, player.CHEST_INVENTORY):
			return False
	...
	def ShopSafebox_SetValue(self, key , *args):
		...
		if key in ("id", "vnum", "count", 'trans', 'cheque', 'evolution'):
			elm[key] = args[0]
	...
	def	OfferList_ItemSetValue(self, key , *args):
		...
		if key in ("id", "vnum", "count", "owner", "price", 'trans', 'evolution'):
			elm[key] = args[0]
	...
	if ENABLE_WEAPON_EVOLUTION_SYSTEM:
		def AuctionList_SetItemEvolution(self, evolution):
			elm = self.AuctionListInfo[-1]['item']
			elm['evolution'] = evolution
	...
	if ENABLE_WEAPON_EVOLUTION_SYSTEM:
		def MyAuction_SetItemEvolution(self, evolution):
			elm = self.MyAuctionInfo[-1]['item']
			elm['evolution'] = evolution
	...
	if ENABLE_WEAPON_EVOLUTION_SYSTEM:
		def OpenAuction_SetItemEvolution(self, evolution):
	...
	def __OnSetCreateAuctionSlot(self, win,slot):
		...
		if ENABLE_WEAPON_EVOLUTION_SYSTEM:
			info.update({ 'evolution' : player.GetItemEvolution(win,slot) })
	...
	def __SetTooltip(self,info):
		...
		if ENABLE_WEAPON_EVOLUTION_SYSTEM:
			self.itemTooltip.AddItemData(info["vnum"], sockets, attrs, evolution = info.get('evolution',0))
		else:
	...
	def __OnUpdateCreateAuctionPage(self):
		...
		# patch create auction slot check
