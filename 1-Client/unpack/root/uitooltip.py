...
#		)
	...
	def SetInventoryItem(self, slotIndex, window_type = player.INVENTORY):
		...
		if item.GetItemType() == item.ITEM_TYPE_GIFTBOX:
			self.AppendSpace(5)
			self.AppendTextLine("|Eemoji/key_ctrl|e + |Eemoji/key_rclick|e - 200 Sandýk Açma")
			self.AppendTextLine("|Eemoji/key_alt|e + |Eemoji/key_rclick|e - Sandýk Önizle")
	...
	def AppendMetinWearInformation(self):
		...
		flagList = (item.IsWearableFlag(item.WEARABLE_BODY),
					...
					item.IsWearableFlag(item.WEARABLE_PENDANT))#ENABLE_PENDANT_SYSTEM
