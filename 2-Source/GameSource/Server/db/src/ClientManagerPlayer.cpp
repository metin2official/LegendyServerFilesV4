...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		else
		{
			snprintf(szQuery, sizeof(szQuery),
					...
					"FROM item%s WHERE owner_id=%d AND (`window` in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','BELT_INVENTORY','UPGRADE_INVENTORY','BOOK_INVENTORY','STONE_INVENTORY','CHEST_INVENTORY'))",
					GetTablePostfix(), pTab->id);
	...
	else
	{
		...
		snprintf(queryStr, sizeof(queryStr),
			...
			"statistics_duels_won, statistics_duels_lost, statistics_bosses_kills, statistics_stones_kills, statistics_mobs_kills "
			", top_damage "
			#endif
...
void CClientManager::__RESULT_PLAYER_DELETE(CPeer *peer, SQLMsg* msg)
{
	...
	if (msg->Get() && msg->Get()->uiNumRows)
	{
		...
		snprintf(queryStr, sizeof(queryStr), "DELETE FROM item%s WHERE owner_id=%d AND (`window` in ('INVENTORY','EQUIPMENT','DRAGON_SOUL_INVENTORY','BELT_INVENTORY','UPGRADE_INVENTORY','BOOK_INVENTORY','STONE_INVENTORY','CHEST_INVENTORY'))", GetTablePostfix(), pi->player_id);
		delete CDBManager::instance().DirectQuery(queryStr);
