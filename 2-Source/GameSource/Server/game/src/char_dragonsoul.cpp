...
bool CHARACTER::DragonSoul_ActivateDeck(int deck_idx)
{
	...
	if (iPulse - GetDsLoadTime() < PASSES_PER_SEC(10))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bunu Yapabilmek icin 10 Saniye Beklemelisiniz."));
		return false;
	}
#endif
	...
	SetDsLoadTime(); //ENABLE_TIME_CONTROL
