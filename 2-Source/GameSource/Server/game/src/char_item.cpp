...
{
	if (nullptr == pItem || !pItem->IsUpgradeItem())
		return -1;
...
{
	if (nullptr == pItem || !pItem->IsStone())
		return -1;
...
{
	if (nullptr == pItem || !pItem->IsChest())
		return -1;
	BYTE bSize = pItem->GetSize();
	for (int i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
...
//@DsProject45_END
...
#ifdef ENABLE_TILSIM_EFSUNLAMA
				else if (item->GetVnum() == TILSIM_EFSUN)
				{
			...
			case USE_ADD_ATTRIBUTE:
				if (ITEM_COSTUME == item2->GetType())// @fixme
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("속성을 변경할 수 없는 아이템입니다."));
					return false;
				}
				...
				if (item2->IsExchanging())// @fixme
				{
					return false;
				}
				if (item2->IsEquipped())// @fixme
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("속성을 변경할 수 없는 아이템입니다."));
					return false;
				}
...
#ifdef ENABLE_TILSIM_EFSUNLAMA
					if (item->GetVnum() == TILSIM_ARTIRMA)
					{
						if (item2->GetType() == ITEM_ARMOR && item2->GetSubType() == ARMOR_PENDANT)
						{
							item2->AddAttribute();
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
...
#ifdef ENABLE_TIME_CONTROL
	int iPulse = thecore_pulse();

	if (iPulse - GetEquipLoadTime() < PASSES_PER_SEC(1))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bunu Yapabilmek ?cin 1 Saniye Beklemelisiniz."));
		return false;
	}
#endif
	...
	if (GetWear(WEAR_COSTUME_BODY))
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Kostum varken bu esyalari giyemezsin"));
			return false;
		}
	}
	// @fixme210 END
	...
	SetEquipLoadTime(); //ENABLE_TIME_CONTROL
	return true;
}
...
void CHARACTER::RemoveSpecifyItem(DWORD vnum, DWORD count)
{
	...
	{
		if (nullptr == GetUpgradeInventoryItem(i))
			continue;
	...
	for (UINT i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
	{
		if (nullptr == GetBookInventoryItem(i))
			continue;
	...
	for (UINT i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
	{
		if (nullptr == GetStoneInventoryItem(i))
			continue;
	...
	for (UINT i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
	{
		if (nullptr == GetChestInventoryItem(i))
			continue;
