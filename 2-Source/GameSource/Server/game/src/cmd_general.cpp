...
ACMD(do_pvp)
{
	...
	switch (ch->GetMapIndex()) // @fixme212
	{
	case 113: // Ox
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu haritada bu islemi gerceklestiremezsiniz."));
		return;
	}
	char arg1[256];
	one_argument(argument, arg1, sizeof(arg1));
	...
	if (ch->GetWear(WEAR_COSTUME_MOUNT))
	{
		...
		if (mount->GetTransmutation())
		{
			...
			if (itemTable)
				...
				mobVnum = mount->GetValue(4);
		}
		else
			mobVnum = mount->GetValue(4);
#else
		if (mount->GetValue(4) != 0)
			mobVnum = mount->GetValue(4);
#endif
...
ACMD(do_ride)
{
	...
	if (ch->GetWear(WEAR_BODY)) {
		...
		if (armor && (armor->GetVnum() >= 11901 && armor->GetVnum() <= 11904)) {
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu kiyafet varken ata binemezsin."));
			return;
		}
	}
	...
	if (ch->GetWear(WEAR_COSTUME_MOUNT))
	{
		...
		{
			mobVnum = pkItemTransmutation->alValues[1];
		}
		else
		{
			if (mount->GetValue(4) != 0)
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Once ati cagir."));
}
#else
ACMD(do_ride)
{
